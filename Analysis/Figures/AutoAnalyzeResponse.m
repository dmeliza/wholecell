function [pre, pst] = AutoAnalyzeResponse(pre_dir, post_dir, fid)
...
for i = 1:length(pre)
    if nargin > 2
        printdifference(fid, sprintf('R%d: (%3.0f -> %3.0f ms)',i,...
            pre(i).t_peak*1000,post(i).t_peak*1000),...
...
function [results] = analyzedirectory(fid);
...
FILTER_PEAK     = 50;      % lowpass cutoff for finding peak of response
FILTER_ORDER    = 3;
...
for ifile = 1:length(dd)
    ...
    % search for crossing the threshold in the filtered data
    ...
    if DEBUG_LOC
        figure,plot(time,avg);
        hline(mu - sigma * THRESH_ONSET);
    ...
    if iselectrical
        sel_artifact= time>-0.0015 & time <0.0015;
    ...
    results = struct('resp',response,'ir',ir,'sr',sr,'time',at,'units',units,...
        't_peak',num2cell(t_peak),'t_onset',num2cell(t_onset));
end
